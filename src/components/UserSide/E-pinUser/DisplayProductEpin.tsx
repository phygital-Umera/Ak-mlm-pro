...
import {useGetProductEpins} from '@/lib/react-query/Customer/epin';
import {useAuthContext} from '@/context/AuthContext';
import GenericTable from '@/components/Forms/Table/GenericTable';
import Loader from '@/components/common/Loader';
import toast from 'react-hot-toast';

const DisplayProductEpin: React.FC = () => {
  const {user} = useAuthContext();
  const {data: epinsData, isLoading, error} = useGetProductEpins();
    ...
    <div>
      <div>
        <GenericTable
          ...
          onCopy={(item: {count: string}) => {
            navigator.clipboard.writeText(item.count);
...
export default DisplayProductEpin;
